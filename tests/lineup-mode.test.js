...
test("marcador minimalista sem foto", () => { const html = active({ nomesResolvidos: ["Betão"] }).elements.pitch.children.map(x => x.innerHTML).join(" "); match(html, /chip-dot/); equal(html.includes("<img"), false); equal(html.includes("betao.webp"), false); });
